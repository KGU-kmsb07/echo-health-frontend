...
import { MOCK_USER, MOCK_RISKS, MOCK_PLAN, MOCK_BENEFITS } from "../mock/mockData";
...
export function HealthProvider({ children }) {
  const [user, setUser] = useState(() => {
    const saved = localStorage.getItem("echo-health-user");
    if (saved) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem("echo-health-user", JSON.stringify(user));
  }, [user]);

  const [risks, setRisks] = useState(() => {
    const saved = localStorage.getItem("echo-health-risks");
    if (saved) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem("echo-health-risks", JSON.stringify(risks));
  }, [risks]);
  ...
  const updatePlan = (newPlan) => {
    if (newPlan === null) {
      setPlan(null);
    } else if (Array.isArray(newPlan)) {
      setPlan({ data: newPlan, generatedAt: Date.now() });
    } else if (newPlan && newPlan.data) {
      setPlan({
        data: newPlan.data,
        generatedAt: newPlan.generatedAt || Date.now()
      });
    } else {
      setPlan(newPlan);
    }
  ...
  const [goalSteps, setGoalSteps] = useState(() => {
    const saved = localStorage.getItem("echo-health-goal-steps");
    return saved ? Number(saved) : 8000;
  });

  useEffect(() => {
    localStorage.setItem("echo-health-goal-steps", String(goalSteps));
  }, [goalSteps]);

  const updateGoalSteps = (steps) => {
    setGoalSteps(Number(steps) || 8000);
  };
  ...
  useEffect(() => {
    if (user && hasOnboarded && !originalUser) {
      setOriginalUser(user);
    }
  }, [user, hasOnboarded, originalUser]);

  const updateUser = (changedData) => {
    setUser(prev => {
      const updated = { ...prev, ...changedData };
      const calculated = calculateHealthData(updated, updated.age);
      return {
        ...updated,
        bmi: calculated.bmi,
        healthScore: calculated.healthScore,
        healthAge: calculated.healthAge,
        personaTags: [updated.gender, `${updated.age}세`, updated.region, updated.district, updated.smoking, calculated.bmi >= 25 ? "비만" : "정상"]
      };
    });
  };
  ...
  useEffect(() => {
    if (window.EchoHealthBridge) {
      window.receiveHealthData = (jsonString) => {
        try {
          const parsed = JSON.parse(jsonString);
          setWearData(parsed);
        } catch (e) {
          console.error("Error parsing health data from bridge", e);
        }
      };
    }
    return () => {
      if (window.receiveHealthData) {
        delete window.receiveHealthData;
      }
    };
  }, []);

  const addNotification = (title, icon, targetScreen, targetTab) => {
    if (!notifEnabled) return;
