...
function NewResultScreen({ setScreen, back }) {
  ...
  const prevUser = firstResult?.user || { healthScore: 68, healthAge: 29, bmi: 24.1, waist: 85, bloodPressure: { systolic: 128, diastolic: 82 }, sleep: 6, exercise: "주 1~2회" };
  const prevRisks = firstResult?.risks || { diabetes: 31, hypertension: 25, metabolic: 38, obesity: 46 };
  
  const currUser = newResult?.user || prevUser;
  const currRisks = newResult?.risks || prevRisks;

  const scoreDiff = currUser.healthScore - prevUser.healthScore;
  ...
  const formatDelta = (label, prevVal, currVal) => {
    ...
    // 위험도 지표 (낮아질수록 양호)
    if (diff < 0) return { text: `↘ -${Math.abs(diff)}%p`, color: "#10B981" };
    if (diff > 0) return { text: `↗ +${diff}%p`, color: "#EF4444" };
    return { text: "유지", color: "#9CA3AF" };
  };
        ...
        <div style={{ padding: 16 }}>
              ...
              <div style={{ position: "relative", width: 70, height: 70 }}>
                <svg width="70" height="70" viewBox="0 0 70 70">
                  ...
                  <circle cx="35" cy="35" r="28" fill="none" stroke="#10B981" strokeWidth="7"
                    strokeDasharray={`${2 * Math.PI * 28 * (currUser.healthScore / 100)} ${2 * Math.PI * 28 * (1 - currUser.healthScore / 100)}`}
                    strokeLinecap="round" transform="rotate(-90 35 35)" />
                </svg>
                <span style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", fontWeight: 700, fontSize: 14 }}>{currUser.healthScore}</span>
              </div>
              <div>
                <div style={{ display: "flex", gap: 6, alignItems: "baseline" }}>
                  <span style={{ fontSize: 36, fontWeight: 800, color: "#10B981" }}>{currUser.healthScore}</span>
          ...
          <div style={S.card}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              ...
              <tbody>
                {[
                  { label: "건강 점수", prev: `${prevUser.healthScore}점`, curr: `${currUser.healthScore}점`, valPrev: prevUser.healthScore, valCurr: currUser.healthScore },
                  { label: "건강 나이", prev: `${prevUser.healthAge}세`, curr: `${currUser.healthAge}세`, valPrev: prevUser.healthAge, valCurr: currUser.healthAge },
                  { label: "당뇨 위험", prev: `${prevRisks.diabetes}%`, curr: `${currRisks.diabetes}%`, valPrev: prevRisks.diabetes, valCurr: currRisks.diabetes },
                  { label: "고혈압 위험", prev: `${prevRisks.hypertension}%`, curr: `${currRisks.hypertension}%`, valPrev: prevRisks.hypertension, valCurr: currRisks.hypertension },
          ...
          {[
            { label: "BMI", val: `${currUser.bmi}`, status: currUser.bmi >= 25 ? "비만" : currUser.bmi >= 23 ? "과체중" : "정상", color: currUser.bmi >= 23 ? "#F59E0B" : "#10B981", pct: Math.min(100, Math.round(currUser.bmi / 40 * 100)) },
            { label: "혈압", val: `${currUser.bloodPressure.systolic}/${currUser.bloodPressure.diastolic}mmHg`, status: currUser.bloodPressure.systolic >= 130 ? "주의" : "정상", color: currUser.bloodPressure.systolic >= 130 ? "#F59E0B" : "#10B981", pct: Math.min(100, Math.round(currUser.bloodPressure.systolic / 180 * 100)) },
            { label: "운동", val: currUser.exercise, status: "양호", color: "#10B981", pct: 75 },
            { label: "수면", val: `${currUser.sleep}h`, status: "양호", color: "#10B981", pct: Math.round(currUser.sleep / 10 * 100) },
          ].map(item => (
