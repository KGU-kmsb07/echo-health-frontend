...
function FutureScreen({ setScreen, back }) {
  ...
  const items = [
    { label: "당뇨",     now: risks?.diabetes ?? null,     then: future.diabetes },
    { label: "고혈압",   now: risks?.hypertension ?? null, then: future.hypertension },
    { label: "대사증후군", now: risks?.metabolic ?? null,  then: future.metabolic },
    { label: "비만",     now: risks?.obesity ?? null,      then: future.obesity },
  ];
      ...
      <div style={S.scrollArea}>
        <div style={{ padding: 16 }}>
          ...
          <div style={{ background: "#FFFBEB", borderRadius: 10, padding: "10px 14px", marginBottom: 16, fontSize: 12, color: "#92400E" }}>
            📊 KNHANES 2022 회귀 모델 기반 예측치입니다. 생활 습관을 바꾸면 이 결과가 달라질 수 있어요.
          </div>
