...
function OnboardStep4({ next, back }) {
  const { user, hasOnboarded, isEditMode } = useHealth();
  const [smoking, setSmoking] = useState(hasOnboarded ? user?.smoking || "" : "");
  const [drinking, setDrinking] = useState(hasOnboarded ? user?.drinking || "" : "");
  const [exercise, setExercise] = useState(hasOnboarded ? user?.exercise || "" : "");
  ...
  const handleNext = () => {
    ...
    next({ smoking, drinking, exercise });
  };
    ...
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", padding: "20px 24px 32px", background: "#fff" }}>
      <div style={{ display: "flex", alignItems: "center", marginBottom: 20 }}>
        ...
        <div style={{ flex: 1, display: "flex", justifyContent: "center", gap: 6 }}>
          {[0, 1, 2, 3].map(i => <div key={i} style={{ width: i === 3 ? 24 : 8, height: 8, borderRadius: 4, background: "#2563EB" }} />)}
        </div>
      ...
      <button onClick={handleNext} style={S.btn()}>{isEditMode ? "저장하기" : "분석 시작하기"}</button>
    </div>
  );
}
